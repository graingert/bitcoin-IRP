...
\textcite{euro-currency-schemes} describe three different currency schemes: ``Closed currency schemes'', ``Currency schemes with unidirectional
flow'' and ``Currency schemes with bidirectional flow''. Bitcoin is classified as the third type - because with currency exchange systems it is possible to convert bitcoin to fiat currency for example: they are currently trading at \input{price}.
...
%Is the sentence 'the bitcoin protocol allows..' a continutation of the list of differences between the services or something else? TOM: No it's a what the client does, not the protocol.
...
%       and has not (promised) <- (TOM: a promise to pay is the payment) this money to..

Solutions to these problems have already been discussed in the scientific
...
Services such as the UK ``Faster Payments Service''~\cite{guardian-fps}, or
...
users' accounts can be frozen~\cite{mtgox-dwolla,vlad:mtgox-dwolla}.
