...
bitcoins have been generated, and are currently trading at \input{price}.
%Include citation of where and when trading information was taken

<<<<<<< HEAD
The key differences between bitcoin and other digital currencies such as PayPal or Amazon Coins
are: that Bitcoin does not have a central authority; that transactions are
irreversible and can be made pseudo anonymously.  The bitcoin
=======
Bitcoin is distinct from other digital currencies such as central banking
...
%Changed the content to improve readability
%Is the sentence 'the bitcoin protocol allows..' a continutation of the list of differences between the services or something else?
...
%'the participants. If Alice wishes....'
%       You need a signpost to indicate the change in topic from definition to issues with transaction
%       A solution to this would be to say 'This transfer involves trust between the two parties - if Alice...'
%'and has not sent that money to anyone else' 
%       and has not (promised) this money to..
%'This problem is easily solvable'
%       I still don't think this is relevant, I think what you need to say is that you physically exchange currency etc
%'moving the value over a distance'
%       Security is a consideration e.g. highway men! :P
...
currency.
%'to these problems'
%       You need to signpost which problems have been discussed as this is a new paragraph
...
\subsection{Unique Token} \textcite{netcash} discuss one solution,
...
\textcite{eval-privacy} note that it is possible to use a mixing service
